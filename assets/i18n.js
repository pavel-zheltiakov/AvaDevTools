      ...
      { key: 'a11y', h: '🩻 The other tree', sub: 'The tree a screen reader walks, an audit that reads it for you, and every keyboard stop numbered over the running application.',
        steps: [
          { t: '🩻 The other tree',
            ...
            news: [
              { k: 'The accessibility tree', d: 'a third mode beside visual and logical, with the announced name and the role as columns' },
              { k: 'Tab-order overlay', d: 'every keyboard stop numbered over the running application, anything Tab never reaches boxed in red' },
              { k: 'Findings in Problems', d: 'controls announced as nothing but their role, keyboard traps, WCAG 2.1 contrast' },
              { k: 'Accessibility over MCP', d: 'get_tab_order, the new mode in get_tree, the new category in get_problems' },
              { k: 'MCP', d: 'settings in one place, and the tab counts the calls made to each tool' },
              { k: 'UI improvements', d: 'draggable tree columns with every edge drawn, ellipsized Problems rows' },
            ] },
      ...
      { key: 'a11y', h: '🩻 Інше дерево', sub: 'Дерево, яким ходить скрінрідер, аудит, що читає його за вас, і кожна клавіатурна зупинка, пронумерована просто над застосунком.',
        steps: [
          { t: '🩻 Інше дерево',
            ...
            news: [
              { k: 'Дерево доступності', d: 'третій режим поруч із візуальним і логічним, з озвучуваною назвою та роллю в колонках' },
              { k: 'Оверлей порядку Tab', d: 'кожну клавіатурну зупинку пронумеровано над робочим застосунком, усе, куди Tab не потрапляє, обведено червоним' },
              { k: 'Знахідки в Problems', d: 'контроли, що озвучуються лише роллю, клавіатурні пастки, контраст за WCAG 2.1' },
              { k: 'Доступність через MCP', d: 'get_tab_order, новий режим у get_tree, нова категорія в get_problems' },
              { k: 'MCP', d: 'налаштування в одному місці, а вкладка рахує виклики до кожного інструмента' },
              { k: 'Покращення інтерфейсу', d: 'колонки дерева тягнуться, кожну межу видно, довгі рядки Problems згортаються в трикрапку' },
            ] },
      ...
      { key: 'a11y', h: '🩻 另一棵树', sub: '屏幕阅读器走的那棵树、替你读它的审计，以及在运行中的应用上给每个键盘停靠点编上的号。',
        steps: [
          { t: '🩻 另一棵树',
            ...
            newsTitle: '12.1.5 新增',
            news: [
              { k: '无障碍树', d: '视觉树和逻辑树之外的第三种模式，被念出的名称和角色作为两列一同呈现' },
              { k: 'Tab 顺序浮层', d: '每个键盘停靠点都在运行中的应用上编了号，Tab 永远到不了的东西用红框圈出' },
              { k: 'Problems 里的发现', d: '除了角色什么也念不出的控件、键盘陷阱、按 WCAG 2.1 量出的对比度' },
              { k: '无障碍走进 MCP', d: 'get_tab_order，get_tree 的新模式，get_problems 的新类别' },
              { k: 'MCP', d: '设置集中到一处，选项卡按工具分别计数' },
              { k: '界面改进', d: '树的列可拖动、每条边界都画出来，Problems 的长行以省略号收尾' },
            ] },
